...
import (
	...
	"github.com/Part001-R/YaPr-Sprint-6/internal/service/flags"
	gz "github.com/Part001-R/YaPr-Sprint-6/internal/service/gzip"
...
var metrics *MetricsHandlerT
var onceMetrics sync.Once

// Конструктор. Возвращает интерфейс метрик.
func NewMetricsStorage(m *MetricsT, db *MetricsDBT, f flags.ConfigT) MetricsI {
	onceMetrics.Do(func() {
		metrics = &MetricsHandlerT{
			Metrics:             m,
			DB:                  db,
...
var metricsDB *MetricsDBT
var onceMetricsDB sync.Once

// Конструктор. Возвращает хранилище БД.
func NewMetricsDB(db *sql.DB) *MetricsDBT {
	onceMetricsDB.Do(func() {
		metricsDB = &MetricsDBT{
			ptr: db,
			Mu:  sync.RWMutex{},
		}
	})
...
var metricsMemory *MetricsT
var onceMetricsMemory sync.Once

// Конструктор. Возвращает хранилище в памяти.
func NewMetricsMemory() *MetricsT {
	onceMetricsMemory.Do(func() {
		metricsMemory = &MetricsT{
...
func (m *MetricsHandlerT) Middleware(h http.Handler) http.Handler {
		...
		if contentEncoding != "" {
			...
			for _, v := range encodings {
				...
				case "gzip":
					...
					if err != nil {
						logger.Log.Error("Ошибка в NewCompressReader", zap.Error(err))
...
func (m *MetricsHandlerT) MiddlewareAudit(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ow := w
		h.ServeHTTP(ow, r)
